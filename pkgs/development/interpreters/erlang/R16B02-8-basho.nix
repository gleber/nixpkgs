...
mkDerivation rec {
  ...
  preConfigure = ''
    ...
    sed -e s@"/usr/bin/env escript"@$(pwd)/bin/$MACH/escript@g -i lib/diameter/bin/diameterc
  ...
  in ''
    MACH=$(gcc -dumpmachine)
    sed -e s@$(pwd)/bin/$MACH/escript@$out/bin/escript@g -i $out/lib/erlang/lib/diameter-1.4.3/bin/diameterc
  ...
  meta = {
    ...
    maintainers = with pkgs.maintainers; [ mdaiter ];
  };
}
